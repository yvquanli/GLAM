...
def auto_dataset(args):
    ...
    if args.dataset in ['demo', 'bbbp', 'bace', 'sider', 'toxcast', 'tox21']:
    ...
    elif args.dataset in ['esol', 'freesolv', 'lipophilicity', 'physprop_perturbed']:
        from trainer import TrainerMolRegression as Trainer
        if args.dataset == 'physprop_perturbed': from dataset import PertubationDataset as Dataset
        _dataset = Dataset(args.dataset_root, dataset=args.dataset, split_seed=args.split_seed)
        args.out_dim = 1 * _dataset.num_tasks
    else:
...
def auto_metrics(dataset: str):
    metrics = ['valauc', 'auc']
    if dataset in ['esol', 'freesolv', 'lipophilicity', 'physprop_perturbed']: metrics = ['valr2', 'r2']
...
if __name__ == '__main__':
    datasets = ['bbbp', 'bace', 'sider', 'toxcast', 'tox21', 'esol', 'freesolv', 'lipophilicity', 'physprop_perturbed']
